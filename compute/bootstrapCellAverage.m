function bX = bootstrapCellAverage(glm,rsel,bootNo,doi)
% bootstrapCellAverage  Bootstrap-resampled per-cell average responses, one
%                       average per (trial type, sequence position).
%
%   The repetition-index and within-trial ramp regressors (GLM weight columns
%   8:9) are removed from the firing rate before averaging, so the result
%   carries only the sequence and reward structure.
%
%   bX : [4 x nTime x nCell x bootNo]
if nargin < 4, doi = [1,2,3]; end
if nargin < 3, bootNo = 1000; end

co = cell2mat(glm.co);
lco = co(co~=1);
q13 = quantile(lco,1/3);
q23 = quantile(lco,2/3);
...
for bt = 1:bootNo
    for dty = doi
        for s = 1:4
            ...
            bX(s,:,:,bt) = cell2mat(cellfun(@(x1,x2) mean(x1(:,x2),2,'omitnan'),glm.Y(rsel),btsel,'un',0));
        end
    end
end
